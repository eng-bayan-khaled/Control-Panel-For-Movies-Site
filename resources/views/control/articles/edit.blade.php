...
<div class="card text-left">
    <div class="card-header">
        <h3 class="card-title">Edit Of Articles</h3>
    </div>
    <div class="card-body">
    @if ($errors->all())
	...
	{{ Form::open(array('url' =>  '/control/articles/'.$article->id, 'method' => 'put')) }}
	    ...
	    <div class="col-sm-10">
			{{ Form::select('auther_id', App\Models\Admins::pluck('name', 'id'),$article->auther_id, ['class' => 'form-control'])  }}
	    </div>
		</div>
	    ...
	    <div class="col-sm-10">
			{{ Form::select('keywords[]', App\Models\Keywords::pluck('name', 'id'),$keywords,['multiple'=>'multiple','id'=>'sports','class' => 'select2 select2-hidden-accessible','data-placeholder' => 'Select a State','style' => 'width: 100%;', 'data-select2-id' => '9', 'aria-hidden' => 'true'])  }}
	    </div>
		</div>

		<div class="form-group d-flex justify-content-center">
		{{ Form::submit('Save' ,array('class' => 'btn btn-info '))  }}
		{{ Form::button('Cancel' ,array('class' => 'btn btn-default', 'onclick' => "window.location.href='" . url('control/articles') . "'"))  }}
		</div>
	{{ Form::close() }}		
