...
<div class="card text-left">
    <div class="card-header">
        <h3 class="card-title">Edit Of Reviews</h3>
    </div>
    <div class="card-body">
    @if ($errors->all())
	...
	{{ Form::open(array('url' =>  '/control/reviews/'.$review->id, 'method' => 'put')) }}

		<div class="form-group row">
		{{ Form::label('text', 'Text', ['class' => 'col-sm-2 col-form-label'])  }}
		<div class="col-sm-10">
			{{ Form::text('text', $review->text, ['class' => 'form-control'])  }}
		</div>
		</div>
	    ...
	    <div class="col-sm-10">
			{{ Form::select('user_id', App\Models\Users::pluck('name', 'id'),$review->user_id, ['class' => 'form-control'])  }}
	    </div>
		</div>
	    ...
	    <div class="col-sm-10">
			{{ Form::select('movie_id', App\Models\Movies::pluck('title', 'id'),$review->movie_id, ['class' => 'form-control'])  }}
	    </div>
		</div>
	    ...
	    <div class="col-sm-10">
			{{ Form::select('episode_id', App\Models\Episodes::pluck('title', 'id'),$review->episode_id, ['class' => 'form-control'])  }}
	    </div>
		</div>
